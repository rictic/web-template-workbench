...
suite('until directive', () => {
    ...
    test('renders a nothing fallback to an interpolated attribute', async () => {
        const promise = Promise.resolve('foo');
        render(html `<div test="value:${until(promise, nothing)}"></div>`, container);
        assertContent('<div></div>');
        await promise;
        assertContent('<div test="value:foo"></div>');
    });
    ...
    suite('disconnection', () => {
        ...
        test('the same promise can be rendered into two until instances', async () => {
            ...
            render(html `<div>${until(promise, 'unresolved1')}</div><span>${until(promise, 'unresolved2')}</span>`, container);
    ...
    memorySuite('memory leak tests', () => {
        test('tree with until cleared while promise is pending', async () => {
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                const promise = new Promise((r) => resolvers.push(r));
                // Render the until into a `<span>` with a 10kb expando, to exaggerate
                // when DOM is not being gc'ed
                render(template(html `<span .p=${big()}>${until(promise)}</span>`), container);
            ...
            assert.isAtMost(performance.memory.usedJSHeapSize, heap * 1.5, 'memory leak detected');
        });
    });
});
