...
suite('asyncReplace', () => {
    ...
    suite('disconnection', () => {
        ...
        test('does not render when newly rendered while disconnected', async () => {
            const component = (value) => html `<p>${value}</p>`;
        ...
        test('the same promise can be rendered into two asyncReplace instances', async () => {
            const component = (iterable) => html `<p>${asyncReplace(iterable)}</p><p>${asyncReplace(iterable)}</p>`;
    ...
    memorySuite('memory leak tests', () => {
        test('tree with asyncReplace cleared while iterables are pending', async () => {
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                render(template(html `<span .p=${big()}>${asyncReplace(iterable)}</span>`), container);
            ...
            assert.isAtMost(performance.memory.usedJSHeapSize, heap * 2, 'memory leak detected');
        });
    });
});
